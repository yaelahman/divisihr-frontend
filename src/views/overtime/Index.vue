                ...
                <div class="card-body">
                  <div class="table-responsive">
                    <table id="datatable1" class="display nowrap w-100">
                      ...
                      <tbody>
                        <tr style="text-align: center" v-for="(val, index) in overtimes" :key="index">
                          <td>{{ index + 1 }}</td>
                          ...
                          <td>
                            <div class="btn-group">
                              ...
                              <button v-if="val.t_absensi_catatan != null" type="button" class="btn btn-sm btn-light"
                                data-bs-toggle="modal" data-bs-target="#ModalCatatan" @click="Modal(val, 'end')">
          ...
          <form @submit.prevent="updateAbsensi()">
            ...
            <div class="modal-body">
              ...
              <div class="form-group mt-2">
                ...
                <input type="text" readonly value="Lembur" class="form-control" />
              </div>
              <div class="form-group mt-2">
          ...
          <div class="modal-body">
            <div class="row">
              ...
              <div v-else class="col-sm-6">
                <label class="mb-2 text-danger">Belum Lembur Selesai</label>
              </div>
            </div>
          </div>
          <div class="modal-footer">
  ...
  methods: {
    ...
    filterRangeDate() {
      if (this.filterType == 'show') {
        ...
        Swal.fire({
          title: "Ekspor Lembur",
          text: 'Ingin Ekspor Lembur Dengan Tanggal ' + this.search.startDate + ' Sampai ' + this.search.endDate + ' ?',
          icon: 'warning',
          ...
          if (resp.isConfirmed) {
            window.location.href = `${window.location.origin}/exports/lembur?start_date=${this.search.startDate}&end_date=${this.search.endDate}`
          }
        })
      }
    },
    ...
    loadAbsensiLembur() {
      this.$Progress.start();
      axios
        .get(env.VITE_API_URL + "overtime", { params: this.search })
        .then((response) => {
          this.$Progress.finish();
